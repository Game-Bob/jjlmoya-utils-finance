...
export const content: ToolLocaleContent<ProfitabilityUI> = {
  ...
  seo: [
    ...
    {
      type: 'paragraph',
      html: '<strong>ROI</strong> показывает общий процент прибыли, но игнорирует время. <strong>CAGR</strong> лучше подходит для долгосрочного анализа, потому что переводит результат в средний годовой темп сложного роста и позволяет сравнивать вложения с разным сроком. Для корректного сравнения начальная и конечная стоимость должны относиться к одному периоду и учитывать одинаковые комиссии.',
    },
