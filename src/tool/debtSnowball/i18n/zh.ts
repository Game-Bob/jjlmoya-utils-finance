...
export const content: ToolLocaleContent<DebtSnowballUI> = {
  ...
  { type: 'paragraph', html: "结果可用于规划和比较方案，但不能替代官方计算或专业建议。雪球法优先消除最小余额，适合需要快速看到进展的人；雪崩法优先偿还高利率债务，可能节省更多利息。请用相同的余额、利率和额外预算比较两种方案。" },
  { type: 'paragraph', html: "使用前请结合计算器显示的假设一起解读结果。雪球法先处理最小债务，能够提供清晰的阶段目标，但不一定产生最低利息成本；请根据预算、利率和执行难度比较不同还款顺序。" },
  { type: 'paragraph', html: "请检查输入值、单位、四舍五入方式、日期和司法辖区，因为每项都可能改变估算。还应确认每笔债务的最低还款额、利率和余额来自最新账单；利率变化、逾期费用或新增借款会让原来的还款时间线失效。" },
  ...
  { type: 'paragraph', html: "使用前请结合计算器显示的假设一起解读结果。" },
  { type: 'paragraph', html: "请检查输入值、单位、四舍五入方式、日期和司法辖区，因为每项都可能改变估算。" },
  { type: 'paragraph', html: "结果可用于规划和比较方案，但不能替代官方计算或专业建议。请每月更新余额、利率、最低还款额和费用；新的消费、逾期费用或合同变化都会改变还款期限。若无法支付最低还款，请尽早联系贷款机构并制定可执行的预算。" },],
  ui: {
    labelTitle: '彻底自由',
