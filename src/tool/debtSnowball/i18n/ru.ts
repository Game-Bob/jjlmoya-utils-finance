...
export const content: ToolLocaleContent<DebtSnowballUI> = {
  ...
  seo: [
    ...
    {
      type: 'paragraph',
      html: 'Жить в долгах тяжело. <strong>Метод "снежного кома"</strong> ставит первой самую маленькую задолженность, чтобы быстрее получить видимый результат и вернуть контроль над планом выплат. Это не всегда самый дешёвый метод по процентам, но короткие промежуточные цели помогают сохранять мотивацию. Регулярно обновляйте баланс, ставки и минимальные платежи, поскольку старые данные меняют срок и стоимость плана.',
    },
