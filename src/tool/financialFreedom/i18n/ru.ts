...
export const content: ToolLocaleContent<FinancialFreedomUI> = {
  ...
  { type: 'paragraph', html: "Используйте результат для планирования и сравнения сценариев; он не заменяет официальный расчёт или консультацию специалиста." },
  { type: 'paragraph', html: "Перед использованием интерпретируйте результат вместе с допущениями, указанными калькулятором. Включите в расходы аренду, питание, транспорт, страховки и кредиты, чтобы оценка отражала реальную стоимость жизни." },],
  faq,
  bibliography: [
    {
