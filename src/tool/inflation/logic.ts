...
export class InflationCalculator {
  ...
  static calculate(amount: number, startYear: number, endYear: number = 2026): InflationResult {
    if (startYear === endYear) {
      return { finalAmount: amount, percentChange: 0, inflationRate: 0 };
    }
    ...
    for (let year = start + 1; year <= end; year++) {
      ...
      totalInflationRate += inflation;
    }

    const finalAmount = startYear < endYear
      ? Number((amount * multiplier).toFixed(2))
      : Number((amount / multiplier).toFixed(2));

    const percentChange = ((finalAmount - amount) / amount) * 100;

    return {
      finalAmount,
      percentChange: Number(percentChange.toFixed(1)),
      inflationRate: Number(totalInflationRate.toFixed(1)),
    };
  }

  static getAvailableYears(): number[] {
    return Object.keys(this.INFLATION_DATA)
      .map(Number)
      .sort((a, b) => a - b);
  }
}
