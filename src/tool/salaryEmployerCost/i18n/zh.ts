...
export const content: ToolLocaleContent<SalaryEmployerCostUI> = {
  ...
  seo: [
    ...
    { type: 'paragraph', html: "企业需支付税前薪资加上企业承担的社保费用。以3万欧元年薪的正规办公岗为例，2026年企业总成本约为 39,645 欧元。模型会从税前工资估算员工社保和IRPF预扣，再得到税后收入，并把企业承担的社保费加入总成本。请输入年度或月度金额时保持口径一致，同时确认是12薪还是14薪；发薪次数会改变单次到账金额，但不应改变约定的年度税前总额。" },
    ...
    { type: 'paragraph', html: "仅适用于西班牙。基于2026年通用社保制度、全职工作及现金薪资。计算结果仅供参考。实际到手工资和企业成本还可能受到个人情况、地区税费、福利、奖金、合同类型及规则更新的影响。比较不同薪资方案时，请保持IRPF预扣率、发薪次数、缴费基数和工伤费率一致。12薪或14薪不会改变年度税前总额，但会改变每次发放的平均金额。高薪员工还可能受到缴费上限和阶梯式团结社保费的影响。模型不包含兼职、自雇、特殊奖金或之后的工资调整。请保存计算假设，并用最新工资单或官方计算核对重要决定；税年、合同、费率或个人信息变化后应重新计算。" },
  ],
  faq,
  bibliography,
  howTo,
